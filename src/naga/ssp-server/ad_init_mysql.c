    ...
    for (k = 1; k < 3; k++)
    {
        ...
        for (i = 0; i < MAX_PRIO; i++)
        {
            ...
            list_for_each_entry_safe(pos, next, &(ad_lists[k][i].head), node)
            {
                ...
                printf("adtype:				%d\n", pos->ad->adtype) ;
                printf("prio:				%d\n", pos->ad->prio) ;
                printf("push_all_day:		%llu\n", pos->ad->push_all_day) ;
                printf("push_one_day:		%llu\n", pos->ad->push_one_day) ;
                printf("push_per_user:		%llu\n", pos->ad->push_per_user) ;
                printf("push_user_interval:	%llu\n", pos->ad->push_user_interval) ;
                ...
                printf("ad_reachable:       %d\n", pos->ad->ad_reachable) ;
                printf("ad_tag_method:      %d\n", pos->ad->ad_tag_method) ;
                printf("ad_tag_content:     %s\n", pos->ad->ad_tag_content) ;
...
void * loop_check_status(void * param)
{
    ...
    while (1)
    {
        ...
        printf("%-5s %-10s %-15s %-10s %-10s %-10s %-10s\n",
               "ID",
               "adtype",
               "status",
               "user_max",
        ...
        for (k = 1; k < 3; k++)
        {
            ...
            for (i = 0; i < MAX_PRIO; i++)
            {
                ...
                list_for_each_entry_safe(pos, next, &(ad_lists[k][i].head), node)
                {
                    printf("%-5d %-10s %-15s %-10llu %-10llu %-10llu %-10llu\n",
                           pos->ad->id,
                           pos->ad->adtype == 1 ? "PC" : "MB",
                           push_status[pos->ad->push_status],
                           pos->ad->push_per_user,
                           pos->ad->push_one_day,
                           pos->ad->cnt_push_one_day,
                           pos->ad->cnt_push_all_day) ;
                }
...
int main(int argc, char * argv[ ] )
{
    ...
    {
        printf("mysql_set_character_set failde!") ;
        return 0 ;
    }
    ...
    for (k = 1; k < 3; k++)
    {
        for (i = 0; i < MAX_PRIO; i++)
        {
            ...
            if(mysql_query(&mysql, query) == 0)
            {
                ...
                if(mysql_num_fields(result) < 25)
                {
                    printf("mysql error, mysql_num_fields:%d\n", mysql_num_fields(result)) ;
                    return 0 ;
                }
                ...
                {
                    if(index >= bts_array_size(ad_inuse))
                    {
                        printf("total ads out of %d\n", bts_array_size(ad_inuse)) ;
                        continue ;
                    }
                    ...
                    if(row[21])
                    {
                        struct_ptr->ad_reachable = atoi(row[21]) ;
                    }
                    else
                    {
                        // default reachable TRUE
                        struct_ptr->ad_reachable = 1 ;
                    }

                    // ad_tag_method
                    if(row[22])
                    {
                        struct_ptr->ad_tag_method = atoi(row[22]) ;
                    }
                    else
                    {
                        // default  normal
                        struct_ptr->ad_tag_method = TAG_METHOD_NROMAL ;
                    }

                    // ad_tag_content
                    if(row[23])
                    {
                        struct_ptr->ad_tag_content = strdup(row[23]) ;
                    }

                    // ad_owner_unique_id
                    if(row[24])
                    {
                        struct_ptr->ad_owner_unique_id = strdup(row[24]) ;
                    }
...
ad_struct_t * apply_valid_ad(apply_info_t * info, int times)
{
    ...
    for (i = 0; i < MAX_PRIO; i++)
    {
        ...
        list_for_each_entry_safe(pos, next,  &(ad_lists[adtype][i].head), node)
        {
            if(1)
            {
                if(!pos->ad->ad_reachable)
                {
                    continue ;
                }

                if(pos->ad->ad_tag_method != info->user_tag_method)
                {
                    continue ;
                }
/* 
