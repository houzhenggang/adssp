     ...
     if(mysql_query(&mysql, query) == 0)
	{
    	...
    	while (row = mysql_fetch_row(result)) {
			...
			if(row[16] != NULL)
			{
				...
				struct_ptr->domain_white_hashtb = init_domain_from_file(struct_ptr->domain_white_file);
			}
			if(row[17] != NULL)
			{
				struct_ptr->domain_black_file = strdup(row[17]);
				struct_ptr->domain_black_hashtb = init_domain_from_file(struct_ptr->domain_black_file);				
			}
